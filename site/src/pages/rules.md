...
<script setup>
import { useHeadingHighlight } from '../../.vitepress/page-utils/rules.js'
import RuleType from '../../.vitepress/page-utils/RuleType.vue'
...
### `BIN_FILE_NOT_EXECUTABLE` <RuleType type="error" />
...
### `EXPORTS_DEFAULT_SHOULD_BE_LAST` <RuleType type="error" />
...
### `EXPORTS_GLOB_NO_DEPRECATED_SUBPATH_MAPPING` <RuleType type="error,warning" />
...
This may be downgraded as a warning if the corresponding `/*` pattern is already specified, which will assume this is intentional for backward compatibility.

### `EXPORTS_MODULE_SHOULD_BE_ESM` <RuleType type="error" />
...
### `EXPORTS_MODULE_SHOULD_PRECEDE_REQUIRE` <RuleType type="error" />
...
### `EXPORTS_VALUE_INVALID` <RuleType type="error" />
...
### `FIELD_INVALID_VALUE_TYPE` <RuleType type="error" />
...
### `FILE_DOES_NOT_EXIST` <RuleType type="error" />
...
### `IMPORTS_GLOB_NO_DEPRECATED_SUBPATH_MAPPING` <RuleType type="error,warning" />
...
### `LOCAL_DEPENDENCY` <RuleType type="error" />
...
### `MODULE_SHOULD_BE_ESM` <RuleType type="error" />

The `module` field should be ESM only as per convention.

## Warnings

### `CJS_WITH_ESMODULE_DEFAULT_EXPORT` <RuleType type="warning" />
...
### `DEPRECATED_FIELD_JSNEXT` <RuleType type="warning,suggestion" />
...
This may be downgraded as a suggestion if the `"module"` field is already specified, which will assume this is intentional for backward compatibility.

### `EXPORTS_FALLBACK_ARRAY_USE` <RuleType type="warning" />
...
### `EXPORTS_GLOB_NO_MATCHED_FILES` <RuleType type="warning" />
...
### `EXPORTS_VALUE_CONFLICTS_WITH_BROWSER` <RuleType type="warning" />
...
### `FILE_INVALID_EXPLICIT_FORMAT` <RuleType type="warning" />
...
### `FILE_INVALID_FORMAT` <RuleType type="warning" />
...
### `FILE_INVALID_JSX_EXTENSION` <RuleType type="warning" />
...
### `IMPLICIT_INDEX_JS_INVALID_FORMAT` <RuleType type="warning" />
...
### `IMPORTS_FALLBACK_ARRAY_USE` <RuleType type="warning" />
...
### `IMPORTS_GLOB_NO_MATCHED_FILES` <RuleType type="warning" />
...
### `INVALID_REPOSITORY_VALUE` <RuleType type="warning" />
...
1. If `"repository"` is a string, it must be one of the supported shorthand strings from the docs.
2. If `"repository"` is an object with `"type": "git"`, the `"url"` must be a valid [git URL](https://git-scm.com/docs/git-clone#_git_urls) and can be [parsed by npm](https://github.com/npm/hosted-git-info).
3. The `git://` protocol for GitHub repos should not be used due [security concerns](https://github.blog/security/application-security/improving-git-protocol-security-github/).
4. GitHub or GitLab links should be prefixed with `git+` and postfixed with `.git`. (This is also warned by npm when publishing a package).
...
### `HAS_ESM_MAIN_BUT_NO_EXPORTS` <RuleType type="suggestion" />
...
### `HAS_MODULE_BUT_NO_EXPORTS` <RuleType type="suggestion" />
...
Consider adding the field with the minimum Node.js version you actually support. Tightening this range can be a breaking change if it also used to work (or happened to work) in older Node.js versions.
...
### `USE_EXPORTS_OR_IMPORTS_BROWSER` <RuleType type="suggestion" />
